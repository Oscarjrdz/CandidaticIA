...
export default async function handler(req, res) {
    ...
    if (token !== 'oscar-cat-2024') {
        return res.status(403).json({ error: 'Unauthorized' });
    }

    console.log('🚀 [PROD] Starting bulk categoria sanitization...');
    ...
    for (const key of keys) {
        try {
            ...
            const value = candidate.categoria;
            if (value === undefined || value === null || String(value).trim() === '') {
                await updateCandidate(candidateId, { categoria: 'Ayudante' });
                updatedCount++;
            } else {
                skippedCount++;
            }
