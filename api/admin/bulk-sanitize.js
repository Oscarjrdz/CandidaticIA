...
export default async function handler(req, res) {
    ...
    if (token !== 'oscar-sanitize-2024') {
        return res.status(403).json({ error: 'Unauthorized' });
    }

    console.log('🚀 [PROD] Starting bulk employment sanitization...');
    ...
    for (const key of keys) {
        try {
            ...
            const value = candidate.tieneEmpleo;
            if (value === undefined || value === null || String(value).trim() === '') {
                await updateCandidate(candidateId, { tieneEmpleo: 'No' });
                updatedCount++;
            } else {
                skippedCount++;
            }
