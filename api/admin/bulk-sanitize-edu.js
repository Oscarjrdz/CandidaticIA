...
export default async function handler(req, res) {
    ...
    if (token !== 'oscar-edu-2024') {
        return res.status(403).json({ error: 'Unauthorized' });
    }

    console.log('🚀 [PROD] Starting bulk escolaridad sanitization...');
    ...
    for (const key of keys) {
        try {
            ...
            const value = candidate.escolaridad;
            if (value === undefined || value === null || String(value).trim() === '') {
                await updateCandidate(candidateId, { escolaridad: 'Secundaria' });
                updatedCount++;
            } else {
                skippedCount++;
            }
