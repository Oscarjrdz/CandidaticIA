        ...
        if (activeProjectId) {
            ...
            if (currentStep?.aiConfig?.enabled && currentStep.aiConfig.prompt) {
                ...
                if (advanceBracketsMatch && advanceBracketsMatch[0]) {
                    ...
                    // Only falls to NI Gate when NO more vacancies remain.
                    if (hasExitTag) {
                            ...
                            if (_hasNextVac) {
                                ...
                                if (_nextVac && config) {
                                    ...
                                    const _CIT_CANCEL_ACK = [
                                        ...
                                        `Qué pena, ${_pvFn}, pero lo comprendo 💛 Tenemos otra vacante que quizás te interese más 🎯`,
                                    ...
                                    const _CIT_TEASER = [
                                        ...
                                        `💼 Hay una posición de *${_vacName}* esperando. ¿Te la agendo? ✨`,
                                        `🎯 Podría interesarte *${_vacName}*. ¿Quieres que te agende la cita? 🌸`,
                                        `🚀 Tenemos *${_vacName}* disponible para ti. ¿Te agendo la entrevista? 😊`,
                    ...
                    if (nextStep) {
                        ...
                        if (_nextHasFutureDays) {
                            try {
                                ...
                                await saveMessage(candidateId, { from: 'bot', content: _dayListMsg + '[MSG_SPLIT]' + _dayAskMsg, timestamp: new Date().toISOString() });
                            } catch (_dlErr) {
                        ...
                        if (nextStep.aiConfig?.enabled && nextStep.aiConfig.prompt && !isTerminalStep && !_nextHasFutureDays) {
                            ...
                            try {
                                ...
                                if (nextAiResult?.response_text || nextAiResult?.media_url) {
                                    ...
                                    await saveMessage(candidateId, { from: 'bot', content: safeLogText, timestamp: new Date().toISOString(), mediaUrl: mUrl && mUrl !== 'null' ? mUrl : null });
                                }
                            } catch (e) {
...
Responde ÚNICAMENTE con el número entero de meses. Si evade o no menciona ningún tiempo, responde null.`,
                ...
                const ACCENTED_SI = /(?:^|\s)(ajá|sí|así\s+es)(?:\s|$)/i;
                let expResult = null;
