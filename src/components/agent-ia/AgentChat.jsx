...
import { Send, Loader2, Bot, Wrench, FileText, BrainCircuit, Trash2, ThumbsUp, Puzzle, Coins, Rocket, Users, CheckCircle2, XCircle, Pencil, Zap, HelpCircle, AlertTriangle } from 'lucide-react';
...
const FEED_POLL_MS = 6000; // sondeo del feed de Agent Candidatic (narración de acciones/dudas)
...
// IDs del feed de Agent Candidatic ya narrados en este chat (evita duplicarlos en
// cada sondeo o al remontar el componente al cambiar de sección).
function feedSeenKey() {
    return `${storageKey()}_feed_seen`;
}
function loadSeenFeedIds() {
    try {
        const raw = localStorage.getItem(feedSeenKey());
        const list = raw ? JSON.parse(raw) : [];
...
function saveSeenFeedIds(set) {
    try { localStorage.setItem(feedSeenKey(), JSON.stringify([...set].slice(-300))); } catch { /* noop */ }
}

const FEED_KIND_STYLE = {
    action: { icon: Zap, label: 'Agent Candidatic', border: 'border-orange-200 dark:border-orange-800', bg: 'bg-orange-50 dark:bg-orange-900/10', text: 'text-orange-600 dark:text-orange-400' },
    question: { icon: HelpCircle, label: 'Agent Candidatic — duda', border: 'border-amber-300 dark:border-amber-700', bg: 'bg-amber-50 dark:bg-amber-900/15', text: 'text-amber-700 dark:text-amber-400' },
    error: { icon: AlertTriangle, label: 'Agent Candidatic — error', border: 'border-red-200 dark:border-red-800', bg: 'bg-red-50 dark:bg-red-900/10', text: 'text-red-600 dark:text-red-400' }
};
...
const AgentChat = ({ hasApiKey, model, onAgentsUpdated, onMemoryProposed, onSkillsUpdated, onAgentLiveUpdated }) => {
    ...
    // ─── Feed de Agent Candidatic (narración) ────────────────────────────────
    // El motor automático (agent-attend.js) publica acciones/dudas en un feed server-
    // side SIN que el reclutador pregunte nada. Se sondea y las entradas nuevas se
    // insertan como burbujas del sistema en este mismo chat — "nuestro medio de
    // comunicación" pedido: si el motor tuvo una duda, aparece aquí para resolverla.
    const seenFeedRef = useRef(loadSeenFeedIds());

    const pollFeed = async () => {
        try {
            const data = await agentIAFetch('/api/agent-ia/live-feed');
            const feed = Array.isArray(data.feed) ? data.feed : [];
            const fresh = feed.filter((f) => f?.id && !seenFeedRef.current.has(f.id));
            if (!fresh.length) return;
            fresh.forEach((f) => seenFeedRef.current.add(f.id));
            saveSeenFeedIds(seenFeedRef.current);
            setMessages((prev) => [...prev, ...fresh.map((f) => ({ role: 'assistant', feedEntry: f }))]);
        } catch { /* siguiente sondeo lo intenta de nuevo */ }
    };

    useEffect(() => {
        pollFeed(); // al abrir el chat, trae lo que se haya perdido
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3 min-h-0">
                ...
                {messages.map((m, i) => (
                    <div key={i} className="space-y-1.5">
                        {m.feedEntry ? (() => {
                            const f = m.feedEntry;
                            const style = FEED_KIND_STYLE[f.kind] || FEED_KIND_STYLE.action;
                            const Icon = style.icon;
                            return (
                                <div className="flex justify-start">
                                    <div className={`max-w-[85%] rounded-2xl px-3.5 py-2 border ${style.border} ${style.bg}`}>
                                        <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide mb-1 ${style.text}`}>
                                            <Icon className="w-3.5 h-3.5" /> {style.label}
                                            {f.candidateName && <span className="normal-case font-semibold text-gray-500 dark:text-gray-400">· {f.candidateName}</span>}
                                        </div>
                                        <p className="text-sm leading-relaxed whitespace-pre-wrap break-words text-[#111b21] dark:text-[#e9edef]">{f.text}</p>
                                    </div>
                                </div>
                            );
                        })() : (
                        ...
                        {!m.feedEntry && m.role === 'assistant' && (m.toolCalls > 0 || m.agentsUpdated || m.skillsUpdated || m.usageTokens > 0) && (
