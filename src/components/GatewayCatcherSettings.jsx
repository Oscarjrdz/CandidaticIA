...
import { Database, Copy, Check, Tag, ChevronDown, Save, Loader2, Key, Hash } from 'lucide-react';
...
const GatewayCatcherSettings = ({ showToast }) => {
    ...
    useEffect(() => {
        ...
        // Load credentials
        fetch('/api/settings?type=catcher_credentials')
            .then(res => res.json())
            .then(data => {
    ...
    const handleSaveCredentials = async () => {
        ...
        try {
            const res = await fetch('/api/settings', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type: 'catcher_credentials', data: { instanceId, token } })
            });
            const data = await res.json();
            if (data.success) showToast?.('Credenciales Catcher guardadas', 'success');
    ...
    return (
        <Card title="Webhook Catcher" icon={Database}>
            <div className="space-y-2.5 pb-1">
                ...
                <div>
                    ...
                    <div ref={dropdownRef} className="relative z-50">
                        <button
                            ...
                            style={{ borderColor: (tags.find(t => t.name === selectedTag))?.color || '#8b5cf6', borderWidth: '1.5px' }}
                        >
                            ...
                            <div className="absolute bottom-full left-0 mb-1 w-full bg-white dark:bg-[#111b21] border border-gray-200 dark:border-[#222e35] rounded-md shadow-xl max-h-40 overflow-y-auto z-[60]">
                                {tags.map((tag, idx) => (
                                    <button key={idx} onClick={() => handleTagSelect(tag.name)}
                                        className={`w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-[#202c33] flex items-center ${selectedTag === tag.name ? 'bg-blue-50 dark:bg-blue-900/20 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="a2c8..."
                                className="w-full pl-7 pr-2 py-1.5 bg-[#f0f2f5] dark:bg-[#202c33] border border-gray-200 dark:border-gray-700 rounded text-xs outline-none focus:border-purple-500 dark:text-gray-100 transition-colors font-mono"
                                value={instanceId}
                ...
                <button
                    ...
                    disabled={saving}
                    className="w-full py-1.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
                >
                ...
                <div className="pt-2 border-t border-gray-100 dark:border-gray-800">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <button onClick={handleCopy}
                            className="text-[9px] text-purple-600 hover:text-purple-700 dark:text-purple-400 font-bold flex items-center gap-0.5 bg-purple-50 dark:bg-purple-900/20 px-1.5 py-0.5 rounded transition-colors">
