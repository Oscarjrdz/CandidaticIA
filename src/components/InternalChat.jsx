...
export default function InternalChat({ onlineUsers = [] }) {
    ...
    const [incomingOffer, setIncomingOffer] = useState(null); // { from, fromName, sdp }
    ...
    useEffect(() => {
        ...
        window.addEventListener('sse:internal:webrtc', handle);
        return () => window.removeEventListener('sse:internal:webrtc', handle);
    }, [stopViewing]);
        ...
        {viewing && (
            <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col">
                <div className="flex items-center justify-between px-4 py-3 bg-black/60 shrink-0">
                    ...
                    <button
                        onClick={stopViewing}
                        className="flex items-center gap-1.5 text-white/80 hover:text-white text-sm bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-full transition-colors"
                    >
                    ...
                    {incomingOffer && (
                        <div className="mx-3 mb-2 p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-xl shrink-0">
                            ...
                            <div className="flex gap-2">
                                <button onClick={acceptShare} className="flex-1 text-xs bg-blue-600 text-white rounded-lg py-1.5 font-medium hover:bg-blue-700 transition-colors">
                                    Aceptar
                                </button>
                                <button onClick={() => setIncomingOffer(null)} className="flex-1 text-xs bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg py-1.5 font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                                    Rechazar
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="px-3 pb-3 pt-2 border-t border-gray-100 dark:border-gray-700 shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {recipientOnline && (
                                ...
                                >
                                    {sharing
                                        ? <MonitorOff className="w-3.5 h-3.5 text-white" />
